...
namespace ImageCompresser
{
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.OK) // Test result.
            {
                ...
                try
                {
                    ...
                    label7.Text = "New Image Size: Width " + img.Width + " Height " + img.Height;
                    imageH = img.Height;
                    imageW = img.Width;
                }
                catch (IOException)
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(textBox2.Text, out h) || !int.TryParse(textBox3.Text, out w))
            {
                Console.WriteLine("not integer");
                return;
            }
            string path = Path.ChangeExtension(textBox1.Text, null) + "_compressed.png";
            var image = CreateImage(imageW / w, imageH / h, w, h);
            label7.Text = "New Image Size: Width " + imageW / w + " Height " + imageH / h;
            Console.WriteLine("Done, saving to " + path);
            image.Save(path);
        }
        ...
        private Color GetMostFrequentColor(int x, int y, int w, int h)
        {
        ...
        private Bitmap CreateImage(int w, int h, int blockW, int blockH)
        {
            ...
            for (int x = 0; x < w; x++)
            {
                for (int y = 0; y < h; y++)
                {
                    ...
                    try
                    {
                        Color c = map[x * blockW + blockW/2][y * blockH + blockH/2];
                        pic.SetPixel(x, y, c);
                    } catch (Exception e)
                    {
                    ...
                    label2.Text = "Progress: " + soFar + " / " + w * h;
